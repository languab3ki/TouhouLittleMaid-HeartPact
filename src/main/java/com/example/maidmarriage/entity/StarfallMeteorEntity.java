...
public final class StarfallMeteorEntity extends Entity {
    ...
    private void impact() {
        ...
        impacted = true;
        Vec3 pos = position();
        serverLevel.playSound(null, blockPosition(), SoundEvents.GENERIC_EXPLODE, SoundSource.WEATHER, 1.5F, 0.56F);
        serverLevel.playSound(null, blockPosition(), SoundEvents.AMETHYST_BLOCK_CHIME, SoundSource.WEATHER, 2.2F, 0.48F);
        serverLevel.playSound(null, blockPosition(), SoundEvents.GENERIC_EXPLODE, SoundSource.WEATHER, 0.8F, 1.18F);
    ...
    private Optional<Entity> ownerEntity(ServerLevel level) {
        return ownerId == null ? Optional.empty() : Optional.ofNullable(level.getEntity(ownerId));
    }
